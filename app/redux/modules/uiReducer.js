...
export default function reducer(state = initialImmutableState, action = {}) {
  switch (action.type) {
    ...
    case VIEW_PUB:
      return state.withMutations((mutableState) => {
        return action.id;
        mutableState.set('publisher', action.id);
      });  
    case PUBLISHER_DETAILS:
      return state.withMutations((mutableState) => {
        mutableState.set('publisher', action.publisher);
      }); 
    case PUBLISHER_ANALYTICS:
      return state.withMutations((mutableState) => {
        mutableState.set('publisherAnalytics', action.publisherAnalytics);
      });   
    case PUBLISHER_EMBEDS:
      return state.withMutations((mutableState) => {
        mutableState.set('publisherEmbeds', action.publisherEmbeds);
      });   
    default:
      return state;
  }
